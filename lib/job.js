...
class JobQueue {
  ...
  async _watcher () {
    for (const j of this.pending.jobs) {
      ...
      const maxDiff = (1000 * job.timeout_seconds)
      if (updatedDiff > maxDiff) {
        logger.info(`expiring job '${job.id}' %o (diff=%s,maxDiff=%s)`, job.data, updatedDiff.toString(10), maxDiff.toString(10))
