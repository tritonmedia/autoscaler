...
const Job = {
   ...
   * @type JobData
   */
  data: _.create(JobData),

  // time this job was created
  created_at: new Date(),

  // last time this job was "touched"
  updated_at: new Date(),
...
class JobQueue {
  ...
  async delete (id) {
    ...
    this.pending.contents.delete(job.id)
  ...
  async _watcher () {
    for (const j of this.pending.jobs) {
      const job = j[1]
      if (job === undefined) { // TODO: fix w/ rearch
        continue
      }
      ...
      if (updatedDiff > (1000 * job.timeout_seconds)) {
        logger.info(`expiring job '${job.id}'`)
