...
class Watcher extends events.EventEmitter {
  ...
  async _watcher () {
    ...
    const rabbitMQURL = 'http://user:bitnami@127.0.0.1:15672/api'
    ...
    if (ready !== 0) {
      logger.info(`exchange ${this.jobType} has ${ready} ready message(s)`)
      this.emit('inactive', ready)
    }

    if (unacked !== 0) {
      logger.info(`exchange ${this.jobType} has ${unacked} unacked message(s)`)
